"""Run the 15-scenario workflow evaluation with MockProvider and real persistence."""
...
class EvaluationHarness:
    ...
    def new_case(self, scenario_id: str) -> ScenarioContext:
        ...
        state = self.s.start_encounter_logic(
            ...
            training_goal=f"Workflow evaluation scenario {scenario_id}",
...
def run_scenarios(h: EvaluationHarness) -> list[dict[str, Any]]:
    ...
    def blocked_connect(*args: Any, **kwargs: Any) -> None:
        raise RuntimeError("network disabled by workflow no-API scenario")
...
def _write_outputs(results: list[dict[str, Any]], metrics: dict[str, Any]) -> None:
    ...
    (evaluation_dir / "workflow_metrics.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    with (evaluation_dir / "workflow_metrics.csv").open("w", encoding="utf-8-sig", newline="") as file:
    ...
    (results_dir / "workflow_scenarios.json").write_text(
        json.dumps(results, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    with (results_dir / "workflow_scenarios.csv").open("w", encoding="utf-8-sig", newline="") as file:
        writer = csv.DictWriter(
            file,
            fieldnames=[
                "scenario_id",
    ...
    report_lines.extend(
        [
            ...
            "",
            "- Core rates are calculated from authored workflow scenarios and real persisted logs, not independent clinical trials.",
            "- A safety block counts as a successful scenario outcome when blocking is the expected educational behavior.",
            "- Tool error rate includes deliberately invalid calls and expected safety blocks, so lower is not automatically better in this scenario set.",
    ...
    (evaluation_dir / "workflow_evaluation_report.md").write_text(
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="simupatient-workflow-") as temp_dir:
        database_path = Path(temp_dir) / "workflow-evaluation.db"
        ...
        db_session._engine = None
        get_settings.cache_clear()
    print(f"Workflow evaluation complete: {len(results)} scenarios")
    for name, detail in metrics.items():
