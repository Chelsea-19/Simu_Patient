...
def test_goai_artifacts_cover_all_authored_scenarios_and_real_traces():
    scenarios = _load_json("evaluation/results/goai_scenarios.json")
...
def test_goai_metrics_have_reproducible_counts_and_csv_matches_json():
    payload = _load_json("evaluation/goai_metrics.json")
    ...
    with (ROOT / "evaluation/goai_metrics.csv").open(encoding="utf-8-sig", newline="") as file:
...
def test_metric_calculation_uses_counts_instead_of_fixed_claims():
    from evaluation.run_goai_evaluation import calculate_metrics
...
def test_report_retains_non_perfect_legacy_osce_results():
    report = (ROOT / "evaluation/goai_evaluation_report.md").read_text(encoding="utf-8")
    payload = _load_json("evaluation/goai_metrics.json")
...
def test_deployment_document_keeps_required_mock_command():
    deployment = (ROOT / "docs/deployment.md").read_text(encoding="utf-8")
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    command = "LLM_PROVIDER=mock streamlit run streamlit_app.py"

    assert "pip install -r requirements.txt" in deployment
